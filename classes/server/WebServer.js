...
class func:router {
	@router.makeUrlMap(path, pathLen) {
		fo=Baro.file()
		map=Baro.was().urlMap()
		not(path ) path=Cf.val(System.path(),'/web/router')
		...
		fo.list(path, func(info) {
			while(info.next()) {
				...
				if( name.eq('.','..')) continue;
				if( type=='folder') {
					@router.makeUrlMap(fullPath, pathLen)
					continue;
				}
				fnm=fullPath.value(pathLen)
				relative=fnm.findLast('.').trim()
				not(@router.modifyCheck(fullPath, relative, modifyDt)) continue
				if(relative.find('.xx')) {
					uri=relative.findPos('.xx').trim()
					name=uri.value(1)
					if(name.find('/')) {
						name=name.replace('/','#')
					}
					chk=call(#[
						@RoutePage.${name}(req,param,&uri) {
							fullPath='${fullPath}'
							param.templateEventUse=true;
							src=@template.parse(fileRead(fullPath),'',param);
							req.send(src);
						}
					]);
					if(chk ) {
	...
	@router.modifyCheck(fullPath, relative, modifyDt) {
