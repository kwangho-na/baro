...
class sourceEditorMain {
	...
	setSplitterSize() {
		total=arraySum(splitter.sizes());
		if(total>600) {
			arr=arrayDivid("550,*",total);
		} else {
			arr=arrayDivid("7,3",total)
		}
		splitter.sizes(arr);
		print("set splitter sizes==>", splitter.sizes() );
	}
}
...
class sourceEditorPanel {
	...
	runScript(key) {
		...
		if( key.eq(KEY.Return, KEY.Enter) ) {
			if(sel) {
				...
				editor.move("selectEnd");
			} else {
				line=editor.sp('lineStart').spText("lineEnd");
				editor.move("lineEnd").insert("\r\n");
			}
	...
	keyTemplateInput() {
		...
		data=editor.text("select");
		fc=call("template_$key");
		not(typeof(fc,'function')) this.alert("template_$key 는 등록된 함수가 아닙니다");
		str=fc(data, note, editor);
	...
	setEditorSyntax() {
		node=Cf.getObject("editor","syntax");
		not(node) {
			node=Cf.getObject("editor","syntax",true);
	...
	searchText(input) {
		not(input) input=this.memeber(searchInput);
		str=input.value();
...
class editorSource {
	...
	editorCopyLine( mode) {
		...
		if( this.is('select') ) {
			...
			str=this.move(sp).move(ep,true).text('select');
		} else {
	...
	editorUpPress() {
		...
		if( ep.gt(end) ) ep=end;
		this.move(sp).move(ep, true);
		this.insert('');
		usp=this.pos('up','lineStart'), uep=this.pos('up','lineEnd');
		upLine=this.move(usp).move(uep,true).text('select');
	...
	editorDownPress() {
		...
		if( dep >= end ) return;
		if( dsp < dep ) {
			downLine=this.move(dsp).move(dep,true).text("select");
			len=str.length(), dlen=downLine.length();
			dep+=1;
			this.move(sp).move(dep,true);
